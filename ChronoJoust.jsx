window.ChronoJoust = () => {
  ...
  const LEVEL_CONFIGS = {
    ...
    2: {
      ...
      enemies: [
        ...
        { type: 'dropper', x: 300, y: 0, width: 38, height: 44, velocityY: 0, falling: true, landed: false, alive: true, pulse: 0, despawnTimer: 0, spawnDelay: 0, meltProgress: 0, health: 20 },      // Platform y=600, x=250-390
        { type: 'dropper', x: 450, y: 0, width: 38, height: 44, velocityY: 0, falling: true, landed: false, alive: true, pulse: 0, despawnTimer: 0, spawnDelay: 40, meltProgress: 0, health: 20 },     // Platform y=400, x=400-530
        
        // Mid section droppers
        { type: 'dropper', x: 900, y: 0, width: 38, height: 44, velocityY: 0, falling: true, landed: false, alive: true, pulse: 0, despawnTimer: 0, spawnDelay: 80, meltProgress: 0, health: 20 },     // Platform y=400, x=850-990
        { type: 'dropper', x: 1000, y: 0, width: 38, height: 44, velocityY: 0, falling: true, landed: false, alive: true, pulse: 0, despawnTimer: 0, spawnDelay: 120, meltProgress: 0, health: 20 },   // Platform y=550, x=950-1060
        { type: 'dropper', x: 1320, y: 0, width: 38, height: 44, velocityY: 0, falling: true, landed: false, alive: true, pulse: 0, despawnTimer: 0, spawnDelay: 160, meltProgress: 0, health: 20 },   // Platform y=600, x=1250-1430
        
        // Late section droppers
        { type: 'dropper', x: 1420, y: 0, width: 38, height: 44, velocityY: 0, falling: true, landed: false, alive: true, pulse: 0, despawnTimer: 0, spawnDelay: 200, meltProgress: 0, health: 20 },   // Platform y=400, x=1350-1480
        { type: 'dropper', x: 2000, y: 0, width: 38, height: 44, velocityY: 0, falling: true, landed: false, alive: true, pulse: 0, despawnTimer: 0, spawnDelay: 240, meltProgress: 0, health: 20 },   // Platform y=400, x=1950-2080
        { type: 'dropper', x: 2400, y: 0, width: 38, height: 44, velocityY: 0, falling: true, landed: false, alive: true, pulse: 0, despawnTimer: 0, spawnDelay: 280, meltProgress: 0, health: 20 },   // Platform y=300, x=2300-2430
  ...
  useEffect(() => {
    ...
    const gameLoop = () => {
      ...
      // Golem solid collision - player can stand on top, but blocked from sides
      game.enemies.forEach(e => {
        if (e.alive && e.type === 'golem') {
          // Check if player is landing on top of golem
          if (checkCollision(player, e) && player.velocityY > 0 && player.y + 60 - player.velocityY <= e.y) {
            // Player landing on top - treat as platform
            player.y = e.y - 60;
            player.velocityY = 0;
            player.isJumping = false;
            // Move player with golem
            player.x += e.facingDirection * e.speed;
          } else if (checkCollision(player, e)) {
            // Side collision - push player out based on collision side
      ...
      game.enemies.forEach(e => {
        if (e.alive) {
          ...
          const eHit = e.type === 'echo' ? { x: e.x, y: e.y + e.verticalOffset, width: e.width, height: e.height } : 
                       ...
                       e.type === 'dropper' ? { 
                         x: e.x, 
                         y: e.y + (e.landed ? (e.height * (e.meltProgress || 0)) : 0), // Shift down as it melts
                         width: e.width, 
                         height: e.landed ? e.height * (1 - (e.meltProgress || 0)) : e.height // Shrink height as it melts
                       } : e;
          ...
          if ((e.type === 'drifter' || e.type === 'dropper' || e.type === 'golem' || e.type === 'spider' || e.type === 'slug' || e.type === 'wraith' || e.type === 'hooded' || e.phaseState !== 'invisible') && player.isAttacking && player.attackTimer > 10) {
            const spear = { 
              ...
              y: player.y + 20, // Lowered from 18 to 20 for better vertical alignment
              width: 40, 
              height: 14 // Increased from 10 to 14 for better coverage
            };
            if (checkCollision(spear, eHit)) {
              ...
              if (e.type === 'golem') {
                ...
                if (attackingFromBehind) {
                  e.alive = false;
                  const toPlayerX = (player.x + 15) - (e.x + 17);
                  const launchX = toPlayerX > 0 ? -5 : 5;  // Away from player
                  game.energyDrops.push({
                    x: e.x + 17, y: e.y + 70,
                    velocityY: -3, velocityX: launchX, collected: false, pulse: 0, gravity: 0.2, value: 4, collectionDelay: 0
                  });
                }
                ...
                if (e.health <= 0) {
                  ...
                  const dropValue = e.type === 'spider' ? 5 : 
                                   ...
                                   e.type === 'wraith' ? 3 :
                                   e.type === 'hooded' ? 3 : 2;
                  const toPlayerX = (player.x + 15) - (e.x + 12);
                  const launchX = toPlayerX > 0 ? -5 : 5;  // Away from player
                  game.energyDrops.push({
                    x: e.x + 12, y: dropY + 12,
                    velocityY: -3, velocityX: launchX, collected: false, pulse: 0, gravity: 0.2, value: dropValue, collectionDelay: 0
                  });
                }
              } else {
      ...
      // Energy drops - always process (both normal and boss battles)
      game.energyDrops.forEach(d => {
        if (!d.collected) {
          ...
          if (game.boss.active) {
            // Boss battle: constrain to arena (0-1280 width)
            if (d.x < 8) {
              d.x = 8;
              d.velocityX *= -0.5;
            }
          ...
          // Respawn from ceiling if falls through floor (both normal and boss)
          if (d.y > 900) {
            d.y = -20;
            d.velocityY = 0;
          }
          ...
          // Platform collision - only bounce if actually falling onto platform from above
          const platformSet = game.boss.active ? 
            [
              // Ground with gaps
              { x: 0, y: 700, width: 350, height: 20 },
              { x: 480, y: 700, width: 320, height: 20 },
              { x: 930, y: 700, width: 350, height: 20 },
              { x: 100, y: 600, width: 180, height: 15 },
              { x: 400, y: 600, width: 200, height: 15 },
              { x: 750, y: 600, width: 200, height: 15 },
              { x: 1000, y: 600, width: 180, height: 15 },
              { x: 200, y: 500, width: 160, height: 15 },
              { x: 550, y: 500, width: 180, height: 15 },
              { x: 850, y: 500, width: 180, height: 15 },
              { x: 150, y: 400, width: 140, height: 15 },
              { x: 450, y: 400, width: 150, height: 15 },
              { x: 680, y: 400, width: 150, height: 15 },
              { x: 950, y: 400, width: 140, height: 15 },
              { x: 300, y: 300, width: 120, height: 15 },
              { x: 560, y: 300, width: 140, height: 15 },
              { x: 840, y: 300, width: 120, height: 15 }
            ] : game.platforms;
            
          platformSet.forEach(p => {
            const dropBox = { x: d.x - 8, y: d.y - 8, width: 16, height: 16 };
            if (d.velocityY > 0 && checkCollision(dropBox, p)) {
              // Only bounce if drop is mostly above the platform (not at edge)
              if (d.y - 8 < p.y + 5) {
                d.y = p.y - 8;
                d.velocityY = -1.5;
                d.velocityX *= 0.96;
      ...
      if (comp.active) {
        ...
        if (comp.fireTimer > 30) {
          ...
          if (game.boss.active && !game.boss.defeated) {
            ...
            const dist = Math.sqrt(dx * dx + dy * dy);
            comp.projectiles.push({ x: comp.x, y: comp.y, spawnX: comp.x, spawnY: comp.y, vx: (dx / dist) * 8, vy: (dy / dist) * 8, active: true });
          } else {
            ...
            game.enemies.forEach(e => {
              if (e.alive) {
                // Calculate proper Y position for each enemy type
                const ey = e.type === 'echo' ? e.y + e.verticalOffset : 
                           e.type === 'wraith' && e.verticalOffset !== undefined ? e.y + e.verticalOffset :
                           e.type === 'spider' ? e.y :
                           e.type === 'dropper' && e.landed ? e.y + (e.height * (e.meltProgress || 0)) + (e.height * (1 - (e.meltProgress || 0))) / 2 :
                           e.y;
                const dx = e.x - comp.x;
                const dy = ey - comp.y;
                const dist = Math.sqrt(dx * dx + dy * dy);
                if (dist < closestDist) {
                  closestDist = dist;
                  closestEnemy = { e, ey, dx, dy, dist };
                }
              }
            });
            
            // Fire at closest enemy only
            if (closestEnemy) {
              const { dx, dy, dist } = closestEnemy;
              comp.projectiles.push({ 
                x: comp.x, y: comp.y, 
                spawnX: comp.x, spawnY: comp.y,
                vx: (dx / dist) * 8, 
                vy: (dy / dist) * 8, 
                active: true 
              });
        ...
        comp.projectiles.forEach(p => {
          if (p.active) {
            ...
            // Deactivate after traveling 700 pixels radially
            const traveledX = p.x - p.spawnX;
            const traveledY = p.y - p.spawnY;
            const distanceTraveled = Math.sqrt(traveledX * traveledX + traveledY * traveledY);
            if (distanceTraveled > 700) p.active = false;

            if (!game.boss.active) {
              game.enemies.forEach(e => {
                if (e.alive) {
                  // Skip spawning droppers and invisible hooded only
                  if ((e.type === 'dropper' && e.spawnDelay > 0)) return;
                  if (e.type === 'hooded' && (e.spawnDelay > 0 || e.phaseState === 'invisible')) return;
                  ...
                  const eHitbox = e.type === 'spider' ? { x: e.x - e.width/2, y: e.y - e.height/2, width: e.width, height: e.height } : 
                                  ...
                                  e.type === 'slug' ? { x: e.x, y: e.y - 9, width: 35 + (e.extensionFactor || 0) * 15, height: 18 } :
                                  e.type === 'golem' ? { x: e.x, y: e.y, width: 35, height: 140 } :
                                  e.type === 'dropper' ? { 
                                    x: e.x, 
                                    y: e.y + (e.landed ? (e.height * (e.meltProgress || 0)) : 0),
                                    width: e.width, 
                                    height: e.landed ? e.height * (1 - (e.meltProgress || 0)) : e.height 
                                  } :
                                  { x: e.x, y: ey, width: e.width, height: e.height };
                  
                  if (checkCollision({ x: p.x - 4, y: p.y - 4, width: 8, height: 8 }, eHitbox)) {
                    // Golem: only damage from behind
                    if (e.type === 'golem') {
                      const projectileFromBehind = (e.facingDirection === 1 && p.x < e.x + 17) || 
                                                   (e.facingDirection === -1 && p.x > e.x + 17);
                      if (!projectileFromBehind) return; // No damage from front
                    }
                    
                    p.active = false;
                    
                    if (e.type === 'golem') {
                      // Golem dies in one hit from behind
                      e.alive = false;
                      const toPlayerX = (player.x + 15) - (e.x + 17);
                      const launchX = toPlayerX > 0 ? -5 : 5;
                      game.energyDrops.push({
                        x: e.x + 17, y: e.y + 35,
                        velocityY: -3, velocityX: launchX, collected: false, pulse: 0, gravity: 0.2, value: 4, collectionDelay: 0
                      });
                      ...
                      if (e.health <= 0) {
                        ...
                        const launchX = toPlayerX > 0 ? -5 : 5;  // Away from player
                        game.energyDrops.push({
                          x: e.x + 12, y: dropY + 12, velocityY: -3, velocityX: launchX,
                          collected: false, pulse: 0, gravity: 0.2, value: dropValue, collectionDelay: 0
                        });
                      }
                    } else {
                      // Single-hit enemies die normally (drifter, echo, slug)
                      ...
                      const launchX = toPlayerX > 0 ? -5 : 5;  // Away from player
                      game.energyDrops.push({
                        x: e.x + 12, y: ey + 12, velocityY: -3, velocityX: launchX,
                        collected: false, pulse: 0, gravity: 0.2, value: dropValue, collectionDelay: 0
                      });
                    }
                  }
      ...
      if (!game.boss.active) {
        ...
        game.enemies.forEach(e => {
          if (e.alive && e.type === 'dropper' && e.spawnDelay === 0) {
            ...
            const currentHeight = 44 * (1 - meltProgress);  // 44 instead of 35
            const meltOffset = 44 - currentHeight;
            
            const dropGrad = ctx.createRadialGradient(e.x + 19, e.y + meltOffset + 12, 0, e.x + 19, e.y + meltOffset + 12, 31);  // 19 center, 31 radius
            ...
            ctx.beginPath();
            const topRadius = 19 * (1 - meltProgress * 0.5);  // 19 instead of 15
            if (currentHeight > 19) {
              ctx.arc(e.x + 19, e.y + meltOffset + 19, topRadius, Math.PI, 0, false);
            }
            ctx.lineTo(e.x + 38, e.y + 44);  // 38 width, 44 height
            ctx.lineTo(e.x, e.y + 44);
            ctx.closePath();
            ctx.fill();
            ...
            if (meltProgress < 0.8) {
              ...
              ctx.beginPath();
              ctx.arc(e.x + 19, e.y + meltOffset + 22, 10 * (1 - meltProgress), 0, Math.PI * 2);  // 19 center x, 22 center y, 10 radius
              ctx.fill();
              ctx.shadowBlur = 30;
            }
          }
        ...
        // ENERGY DROPS - render in world coordinates (with camera translation)
        game.energyDrops.forEach(d => {
          if (!d.collected) {
        ...
        if (!game.boss.defeated) {
          ...
          if (!isInvulnerable && teleportAlpha > 0) {
            ...
            if (boss.type === 'bat') {
              // PHANTOM COLONY BAT BOSS (Level 1)
              const centerX = boss.x + 40;
              const centerY = boss.y + 50;
              const time = Date.now() * 0.002;
              const bossPulse = Math.sin(boss.pulse) * 0.5 + 0.5;
              
              // Draw 6 smaller colony bats orbiting (behind main bat)
              for (let i = 0; i < 6; i++) {
                const angle = (i / 6) * Math.PI * 2 + time * 0.6;
                const distance = 90 + Math.sin(time * 2 + i) * 15;
                const x = centerX + Math.cos(angle) * distance;
                const y = centerY + Math.sin(angle) * distance;
                const scale = 0.35;
                
                // Energy connection thread
                ctx.strokeStyle = `rgba(136, 136, 136, ${0.4 + bossPulse * 0.2})`;
                ctx.lineWidth = 2;
                ctx.setLineDash([5, 5]);
                ctx.beginPath();
                ...
                // Small colony bat
                ctx.globalAlpha = 0.7 + bossPulse * 0.2;
                ctx.shadowBlur = 15;
                ctx.shadowColor = '#ff69b4';
                ctx.fillStyle = '#777777';
                
                // Body
                ctx.fillRect(x - 12 * scale, y - 8 * scale, 24 * scale, 20 * scale);
                // Head
                ctx.fillRect(x - 10 * scale, y - 18 * scale, 20 * scale, 12 * scale);
                
                // Ears
                ctx.fillStyle = '#555555';
                ctx.beginPath();
                ctx.moveTo(x - 8 * scale, y - 18 * scale);
                ctx.lineTo(x - 12 * scale, y - 26 * scale);
                ...
                ctx.lineTo(x - 22 * scale, y - 3 * scale);
                ...
                ctx.beginPath();
                ctx.arc(x - 4 * scale, y - 13 * scale, 2 * scale, 0, Math.PI * 2);
                ctx.fill();
                ctx.beginPath();
                ctx.arc(x + 4 * scale, y - 13 * scale, 2 * scale, 0, Math.PI * 2);
                ctx.fill();
              ...
              // Main central bat (leader)
              ctx.shadowBlur = 40;
              ctx.shadowColor = '#ff69b4';
              ctx.fillStyle = '#888888';
              
              // Body
              ctx.fillRect(centerX - 25, centerY - 18, 50, 40);
              
              // Head
              ctx.fillRect(centerX - 18, centerY - 38, 36, 25);
              
              // Ears
              ctx.fillStyle = '#666666';
              ctx.beginPath();
              ctx.moveTo(centerX - 15, centerY - 38);
              ctx.lineTo(centerX - 22, centerY - 52);
              ...
              // Eyes (leader - brighter pink glow)
              ctx.shadowBlur = 30 + bossPulse * 10;
              ctx.shadowColor = '#ff1493';
              ctx.fillStyle = '#ff1493';
              ctx.beginPath();
              ctx.arc(centerX - 10, centerY - 28, 6, 0, Math.PI * 2);
              ctx.fill();
              ctx.beginPath();
              ctx.arc(centerX + 10, centerY - 28, 6, 0, Math.PI * 2);
              ctx.fill();
              ...
              ctx.fillStyle = '#888888';
              ctx.beginPath();
              ctx.arc(centerX - 10, centerY - 28, 3, 0, Math.PI * 2);
              ctx.fill();
              ctx.beginPath();
              ctx.arc(centerX + 10, centerY - 28, 3, 0, Math.PI * 2);
              ctx.fill();
              
              // Battle scars on leader
        ...
        // ENERGY DROPS - render in screen coordinates (no camera translation)
        game.energyDrops.forEach(d => {
          if (!d.collected) {
            const pulseFactor = Math.sin(d.pulse) * 0.5 + 0.5;
            const glowSize = 20 + pulseFactor * 15;
